...
namespace Utils
{
    ...
    inline int32_t sign_extend32(uint32_t opcode, int start_offset, int signed_bit)
    {
        bool is_signed = Utils::is_bit_set(opcode, signed_bit);
        uint32_t unsigned_offset = Utils::get_bits(opcode, start_offset, signed_bit);
   
        uint32_t negative_bitmask = 0xFFFFFFFF << signed_bit;
